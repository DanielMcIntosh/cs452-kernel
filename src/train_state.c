...
static inline void __attribute__((nonnull)) activeroute_exec_steps(TerminalCourier * restrict tc, ActiveRoute * restrict ar, MyReservation * restrict my_reserv, Train * restrict train, int resrv_dist, int cmdtid) {
    ...
    while (ACTIVE_ROUTE_SHOULD_PERFORM_ACTION(ar, resrv_dist, nxt_step_rv_in_range))  { // must perform next actio due to proximity directly or bc the reverse will take a while
        ...
        if (RESERVE_TRACK) {
            bool resrv_successful = reserve_track(&ar->route, ar->idx_resrv, resrv_start, resrv_end, my_reserv, "main reserve_track");
            if (!resrv_successful) {
                //TODO handle this
                break;
            }
...
static inline void __attribute__((nonnull)) activeroute_on_sensor_event(TerminalCourier * restrict tc, ActiveRoute * restrict ar, MyReservation * restrict my_reserv, Train * restrict train, int sensor, int distance, int cmdtid){
...
void __attribute__((noreturn)) task_train_state(int trackstate_tid) {
    ...
    FOREVER{
        ...
        case (NOTIFY_RESERVATION):
        {
            ...
            if (likely(success)) {
                terminal_set_reservations(&tc, &(ts.reservations.blkges[MAX_CONCURRENT_TRAINS]), MAX_CONCURRENT_TRAINS);
            } else {
                Blockage freed;
        ...
        case (NOTIFY_STOPPED):
        {
            ...
            MyReservation my_reserv;
            reservation_to_my_reservation(&my_reserv, &(ts.reservations), a_tr);
